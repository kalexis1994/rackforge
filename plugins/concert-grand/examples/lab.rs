...
/// The laboratory is a Windows program: it detaches into its own console
/// and takes its MIDI from Windows MIDI Services as well as from midir.
#[cfg(windows)]
mod lab {
                ...
                Ok(transport) => {
                    ...
                    match transport.connect(endpoint, move |words, _timestamp| {
                        ...
                        let _ = reader.read(
                            ...
                            0,
                            &mut |_group, packet| {
                                deliver(&packet.data[..usize::from(packet.length)])
                            },
...
#[cfg(not(windows))]
fn main() {
    eprintln!("the Concert Grand laboratory runs on Windows; use lab.cmd there");
    std::process::exit(2);
}
