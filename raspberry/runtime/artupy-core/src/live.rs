...
use artupy_plugin_api::PluginKind;
...
pub fn run(config: LiveConfig) -> Result<()> {
    ...
    // plugin package.
    let plugin =
        unsafe { LoadedPlugin::load(&package, config.binary.as_deref(), &config.resources) }?;
    println!(
...
fn connect_keylab(sender: Sender<MidiEventV1>) -> Result<(MidiInputConnection<()>, String)> {
    let mut midi = MidiInput::new("artupy-core-live")?;
    midi.ignore(Ignore::None);
    let (port, name) = select_keylab_port(&midi)?;
    let connection = midi
        .connect(
            &port,
            "artupy-core-live-input",
