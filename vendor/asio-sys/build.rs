...
fn main() {
    ...
    // RackForge fork: the bindings are generated every time this script runs.
    // Upstream skipped generation when `asio_bindings.rs` already existed, but
    // OUT_DIR outlives changes to this script, so a new entry in the allowlist
    // below never reached a build directory that had bindings from before it
    // -- a cached CI target directory included. This script only runs when
    // one of its inputs changes, so regenerating costs nothing in the common
    // case.
    if is_msvc() {
        invoke_vcvars_if_not_set();
    }
...
fn create_bindings(cpal_asio_dir: &PathBuf) {
    ...
    let bindings = bindgen::Builder::default()
        ...
        // RackForge fork: the driver's own settings window.
