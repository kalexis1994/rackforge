...
and `--edit` also opens the tuning file in the default editor. On first run it writes every knob, with its documentation, to
...
| Bridge loss by string | ∝ (Z_tenor / Z)³, Z = μ·c from the gauge law: A0 0.05 (floored 0.08), A1 0.22, C2 0.33, A2 0.8, C4 1, C7 1.5 (cap) | Loss to the bridge ∝ bridge admittance / string impedance; wound bass strings 10–25× a tenor's mass per metre | Fletcher & Rossing; Conklin Part I | **Moved (0.145)**: BRIDGE_IMPEDANCE_POWER 3, BRIDGE_REFERENCE_GAUGE_M 1.23e-3, GAUGE_TOP_M 0.8e-3. Same factor scales the viscoelastic wire loss. Bass-note partials at 1.6–3.2 kHz now hold 9.7 s early / 10.5 s late (Salamander 5.9/9.0, Pianoteq 5.6/8.0), at 3.2–6 kHz 4.6/5.5 (4.2/6.4, 4.1/5.3); the bass envelope sits inside both references at 0.5–4 s. Tenor and treble untouched. Still open: bass partials at 400–800 Hz decay 4.5 s early where both references hold 10.5. |
| Unison width | 0.45 + 0.45·pos cents × Unison fader × per-note precision (~1.2 cents mid) | tuners prefer 1–2 cents; wider unisons dephase sooner | Kirk 1959; Weinreich 1977 | **Moved (0.146)**: UNISON_WIDTH 0.5 (half the old width). The knee between the coherent, draining decay and the dephased aftersound is the unison's dephasing time: at the old width the tenor's and treble's early decays ran 2–3× slower than both references (treble 400–800 Hz 6.3 s early vs 2.1/1.7; tenor 800–1600 Hz 7.2 vs 3.0/2.6); at half width 2.0 and 3.9, late decays held, envelopes of all three registers inside the references at 0.5–4 s. |
| Bridge loss, two stages | Coherent stage: rate ∝ (L_C4/L) · (Z_tenor/Z)² capped ×4 at the top (A0 0.14, C2 0.3, C4 1, C5 1.7, C6 3, C7 4); dephased stage: impedance law alone, capped ×1.5, with a tenth of the wire's HF loss; horizontal share 0.1 | rate = 2·T·Re(Y)/L: reflections per second times what each reflection leaves at the bridge | Weinreich 1977; Fletcher & Rossing | **Moved (0.147)**: BRIDGE_LENGTH_POWER 1, BRIDGE_REFERENCE_LENGTH_M 0.62, BRIDGE_FACTOR_CAP 4, SLOW_FACTOR_CAP 1.5, SLOW_WIRE_SHARE 0.1, HORIZONTAL_SHARE 0.1, BRIDGE_IMPEDANCE_POWER 2. Scaling the dephased stage with the reflections as well cut the treble aftersound to 1.5 s (references 3–5); with the horizontal share at 0.3, the early windows read the slow stage. Now treble early/late T60 by band: 400–800 Hz 2.1/6.5 (Sal 2.1/9.0, PTQ 1.7/7.6), 1.6–3.2 kHz 2.9/5.0 (2.2/5.4, 1.9/4.0), 3.2–6 kHz 2.5/2.3 (1.6/4.1, 1.5/3.3); treble envelope −29/−36/−52 dB at 0.5/1/2 s (refs −27/−40/−52). Bass 400–800 Hz early 7.4 s (was 3.4; refs 10.5). Open: bass envelope at 1 s −7.8 vs −9…−10.6; tenor at 0.5 s −12.9 vs −9.7. |
| Evenness of the top two octaves | fundamental's level between semitones, D6–F#7 at v100: 4.3 dB rms (was 8.1) | Pianoteq NY Steinway D: 3 dB rms (trial's muted keys excluded) | measured 2026-09-02 | **Moved (0.149)**, three causes found in order: (1) the bridge drain of the coherent stage was normalised by the strike's share per string (a per-note hash) while `cull` sums the strings equally — E7 drained twice as fast as F7 (k 0.051 vs 0.027) and its fundamental sat 12 dB down at 100 ms; now normalised by the number of strings. (2) The board bank above the ribbed-plate knee held 0.6 modes per bandwidth with drawn signs: a Rayleigh transfer with 15–20 dB fades at single frequencies (F6 −21 dB, G6 −3). Now four modes per bandwidth (BOARD_TOP_OVERLAP 4, BOARD_MODES 512 → 363 placed) and one sign above the knee (BOARD_SIGN_TOP_HZ 1477) as a driving point sums: the bank's spread at the fundamentals fell from 8 to 2 dB rms. (3) The strike simulation stood down above G#6 because fewer than four partials sit under 8 kHz; it now simulates whatever the note has (≥ 2). Register decays and envelopes unchanged within 0.5 dB. |
