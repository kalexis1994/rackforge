...
|---|---|
| `crates/rackforge-control-api/` | Protocolo local versionado entre Core y superficies de control. |
| `crates/rackforge-session-api/` | Estado, instancias, comandos, eventos y revisiones independientes de plataforma. |
| `crates/rackforge-plugin-api/` | ABI C versionada, manifiestos y esquema declarativo de parámetros. |
| `crates/rackforge-core/` | Descubrimiento, validación, carga dinámica e instancias. |
| `crates/rackforge-ui/` | Componentes, foco, layout y estilos independientes del hardware. |
...
- Ningún `String`, `Vec`, trait object ni asignador de Rust cruza el límite
  binario.
- `crates/rackforge-plugin-api/include/rackforge_plugin.h` permite implementar plugins en
...
`platforms/raspberry-pi/scripts/install.sh` instala el runtime y sus plugins incluidos mediante
...
`platforms/raspberry-pi/scripts/install-appliance.sh` instala el host privilegiado con socket Unix
...
appliance se ejecuta con
`platforms/raspberry-pi/scripts/install-appliance.sh --optimize`, o por separado
con `platforms/raspberry-pi/scripts/optimize-appliance.sh audit|apply|rollback`. El perfil no
