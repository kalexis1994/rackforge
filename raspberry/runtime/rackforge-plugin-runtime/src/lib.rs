...
impl PortableModule {
    pub fn instantiate(&self) -> Result<PortableInstance> {
        ...
        let capacity = typed::<(), i32>(&instance, &mut store, "rackforge_capacity_samples")?
            .call(&mut store, ())?;
        if capacity <= 0 {
            bail!("wasm-v1 plugin reported an invalid buffer capacity");
        }
        let midi_capacity =
        ...
        Ok(PortableInstance {
            ...
            midi_offset,
            transfer_offset,
            capacity_samples: capacity as usize,
...
pub struct PortableInstance {
    ...
    prepare: TypedFunc<(f64, i32, i32), i32>,
    ...
    fuel_per_call: u64,
    prepared_channels: u32,
    maximum_frames: u32,
}

impl PortableInstance {
    pub fn prepare(&mut self, sample_rate: f64, maximum_frames: u32, channels: u32) -> Result<()> {
        let samples = checked_samples(maximum_frames, channels)?;
        if samples > self.capacity_samples {
            bail!(
                "plugin capacity {} samples is smaller than requested {samples}",
        ...
        check_status(
            self.prepare.call(
                &mut self.store,
                (sample_rate, maximum_frames as i32, channels as i32),
            )?,
            "prepare",
        )?;
    ...
    pub fn process_interleaved(
        ...
        self.process_interleaved_with_midi(input, output, frames, &[])
    }
    ...
    pub fn process_interleaved_with_midi(
        ...
        if self.prepared_channels == 0 || frames == 0 || frames > self.maximum_frames {
            bail!("portable plugin is not prepared for this audio block");
        }
        let samples = checked_samples(frames, self.prepared_channels)?;
        if input.len() != samples || output.len() != samples {
            bail!("audio buffer length does not match frames and channels");
        }
...
fn checked_samples(frames: u32, channels: u32) -> Result<usize> {
    if frames == 0 || channels == 0 {
        bail!("frames and channels must be non-zero");
    }
    (frames as usize)
...
mod tests {
    ...
    const GAIN: &str = r#"
        (module
          ...
          (func (export "rackforge_capacity_samples") (result i32) i32.const 256)
          ...
          (func (export "rackforge_prepare") (param f64 i32 i32) (result i32) i32.const 0)
          ...
          (func (export "rackforge_process") (param $frames i32) (param $channels i32) (param $midi i32) (result i32)
    ...
    fn runs_one_portable_gain_module() {
        ...
        instance.prepare(48_000.0, 64, 2).unwrap();
