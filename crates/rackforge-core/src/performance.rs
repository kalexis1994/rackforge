...
use rackforge_performance_api::{
    LibraryRevision, LiveLocation, LivePerformanceState, PerformanceEdit, PerformanceLibrary,
};
#[cfg(test)]
use rackforge_performance_api::{
...
const LEGACY_BOOTSTRAP_RACK_ID: &str = "rack.imported.current";
const LEGACY_BOOTSTRAP_SONG_ID: &str = "song.imported.current";
const LEGACY_BOOTSTRAP_SETLIST_ID: &str = "setlist.default";
...
impl PerformanceRepository {
    ...
    /// library and the performer builds the first Rack deliberately.
    pub fn load_or_empty(data_root: Option<&Path>) -> Result<Self> {
        ...
        repository.load()?;
        repository.remove_legacy_bootstrap()?;
        repository.prune_ghost_sequencer_tabs()?;
        repository.persist_initialized_marker()?;
        repository
            .library
            .validate()
    ...
    /// Drops deck tabs that hold nothing the library still has.
    ///
    /// A tab whose every seat is empty or points at a pattern that is gone
    /// opens as a nameless "empty" sequencer with no way to know what it
    /// was for. Surfaces once persisted a tab the moment it was created,
    /// before its draft was ever saved, so these accumulated; the surface
    /// no longer does, and the ones already written are swept here.
    fn prune_ghost_sequencer_tabs(&mut self) -> Result<()> {
        ...
        self.library
            .sequencer_tabs
            .retain(|tab| !ghosts.contains(&tab.lane));
    ...
    /// Removes the Rack/Song/Setlist trio generated by early RackForge builds.
    ///
    /// The fixed IDs identify host-created documents, but deletion still
    /// requires their original dependency shape and no references from user
    /// documents. A repurposed document is preserved rather than guessed at.
    fn remove_legacy_bootstrap(&mut self) -> Result<()> {
        let legacy_song_shape = self
            .library
            .songs
            .iter()
            .find(|song| song.id.as_str() == LEGACY_BOOTSTRAP_SONG_ID)
            .map(|song| {
                song.parts
                    .iter()
                    .all(|part| part.rack_id.as_str() == LEGACY_BOOTSTRAP_RACK_ID)
            });
        let removable_setlist = self
            .library
            .setlists
            .iter()
            .find(|setlist| setlist.id.as_str() == LEGACY_BOOTSTRAP_SETLIST_ID)
            .is_some_and(|setlist| {
                legacy_song_shape.unwrap_or(true)
                    && setlist
                        .entries
                        .iter()
                        .all(|entry| entry.song_id.as_str() == LEGACY_BOOTSTRAP_SONG_ID)
            });

        let song_referenced_elsewhere = self.library.setlists.iter().any(|setlist| {
            (!removable_setlist || setlist.id.as_str() != LEGACY_BOOTSTRAP_SETLIST_ID)
                && setlist
                    .entries
                    .iter()
                    .any(|entry| entry.song_id.as_str() == LEGACY_BOOTSTRAP_SONG_ID)
        });
        let removable_song = !song_referenced_elsewhere && legacy_song_shape == Some(true);

        let rack_referenced_elsewhere = self.library.songs.iter().any(|song| {
            (!removable_song || song.id.as_str() != LEGACY_BOOTSTRAP_SONG_ID)
                && song
                    .parts
                    .iter()
                    .any(|part| part.rack_id.as_str() == LEGACY_BOOTSTRAP_RACK_ID)
        });
        let removable_rack = !rack_referenced_elsewhere
            && self
                .library
                .racks
                .iter()
                .any(|rack| rack.id.as_str() == LEGACY_BOOTSTRAP_RACK_ID);

        if !removable_setlist && !removable_song && !removable_rack {
            return Ok(());
        }

        if let Some(root) = &self.root {
            if removable_setlist {
                delete_document(&root.join("setlists"), LEGACY_BOOTSTRAP_SETLIST_ID)?;
            }
            if removable_song {
                delete_document(&root.join("songs"), LEGACY_BOOTSTRAP_SONG_ID)?;
            }
            if removable_rack {
                delete_document(&root.join("racks"), LEGACY_BOOTSTRAP_RACK_ID)?;
            }
        }
        if removable_setlist {
            self.library
                .setlists
                .retain(|setlist| setlist.id.as_str() != LEGACY_BOOTSTRAP_SETLIST_ID);
        }
        if removable_song {
            self.library
                .songs
                .retain(|song| song.id.as_str() != LEGACY_BOOTSTRAP_SONG_ID);
        }
        if removable_rack {
            self.library
                .racks
                .retain(|rack| rack.id.as_str() != LEGACY_BOOTSTRAP_RACK_ID);
        }
        println!("PERFORMANCE_LEGACY_BOOTSTRAP_REMOVED");
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn empty_host_start_removes_the_legacy_default_performance_trio() {
        let root = temporary_root("remove-legacy-bootstrap");
        let repository =
            PerformanceRepository::load_or_bootstrap(Some(&root), bootstrap()).unwrap();
        assert_eq!(repository.library().racks.len(), 1);
        drop(repository);

        let migrated = PerformanceRepository::load_or_empty(Some(&root)).unwrap();
        assert!(migrated.library().racks.is_empty());
        assert!(migrated.library().songs.is_empty());
        assert!(migrated.library().setlists.is_empty());
        assert!(
            !root
                .join("performance/racks/rack.imported.current.json")
                .exists()
        );
        assert!(
            !root
                .join("performance/songs/song.imported.current.json")
                .exists()
        );
        assert!(
            !root
                .join("performance/setlists/setlist.default.json")
                .exists()
        );
        fs::remove_dir_all(root).unwrap();
    }
    ...
    #[test]
    fn first_empty_host_start_does_not_create_a_default_rack() {
        let root = temporary_root("empty-first-start");
        let repository = PerformanceRepository::load_or_empty(Some(&root)).unwrap();
        assert!(repository.library().racks.is_empty());
        assert!(repository.library().songs.is_empty());
